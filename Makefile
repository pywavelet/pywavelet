...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt pywavelet
...
.PHONY: format
format:
	@black src/pywavelet tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/pywavelet tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p pywavelet --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc pywavelet ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/pywavelet*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./pywavelet-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/pywavelet-*-py3-none-any.whl
