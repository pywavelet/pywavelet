...
def from_wavelet_to_time(
    wave_in: Wavelet, nx: float = 4.0, mult: int = 32, dt: Optional[float] = 1
) -> TimeSeries:
...
def from_wavelet_to_freq_to_time(
    wave_in: Wavelet, Nf: int, Nt: int, nx: float = 4.0
) -> TimeSeries:
...
def from_wavelet_to_freq(wave_in, Nf, Nt, dt, nx=4.0):
