...
def transform_wavelet_freq_helper(
    ...
    initial_values = cp.where(
        (f_bins == 0) | (f_bins == Nf),  # Edge cases: DC (f=0) and Nyquist (f=Nf)
    ...
    DX[:, Nt // 2] = initial_values  # Set initial values at the center of the transformation (2 sided FFT)

    # Compute time indices for all offsets around the midpoint
    j_range = cp.arange(1 - Nt // 2, Nt // 2)  # Time offsets (centered around zero)
    ...
    cond1 = (n_range[:, None] + f_bins[None, :]) % 2 == 1  # Odd/even alternation
    ...
    wave[1::2, -1] = cp.real(DX_trans[-1, ::2] * cp.sqrt(2))  # Nyquist component
