...
def phitilde_vec(om: np.ndarray, Nf: int, nx=4.) -> np.ndarray:
...
def phitilde_vec_norm(Nf: int, Nt: int, nx: int) -> np.ndarray:
    ...
    nrm_fctor = np.sqrt((2 * np.sum(u_phit[1:] ** 2) + u_phit[0] ** 2) * 2 * PI/ ND)
    nrm_fctor /= PI ** (3 / 2) / PI

    return u_phit / nrm_fctor


def phi_vec(Nf: int, nx: int = 4., mult: int = 16) -> np.ndarray:
    """get time domain phi as fourier transform of phitilde_vec"""
    insDOM = 1. / np.sqrt(PI / Nf)
    K = mult * 2 * Nf
