...
def get_sampling_frequency_and_duration_from_time_array(
...
def get_sampling_frequency_and_duration_from_frequency_array(
    ...
    sampling_frequency = np.round(
        2 * (number_of_frequencies - 1) / duration, decimals=14
    )
    return sampling_frequency, duration


def create_time_series(
    sampling_frequency: float, duration: float, starting_time: float = 0
) -> np.ndarray:
    """
...
def create_frequency_series(
    sampling_frequency: float, duration: float
) -> np.array:
...
def _check_legal_sampling_frequency_and_duration(
    sampling_frequency: float, duration: float
) -> None:
...
def create_white_noise(sampling_frequency: float, duration: float) -> np.array:
...
def nfft(
    time_domain_strain: np.array, sampling_frequency: float
...
def infft(
    frequency_domain_strain: np.array, sampling_frequency: float
) -> np.array:
...
class PropertyAccessor(object):
    ...
    def __set__(self, instance, value):
        setattr(
            getattr(instance, self.container_instance_name),
            self.property_name,
            value,
        )
