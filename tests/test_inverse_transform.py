...
WNM = np.array([
    [-2.17231658e-16, -2.17231658e-16, 1.82849722e-15, 1.82849722e-15, -1.59693281e-15, -1.59693281e-15,
     -1.14095073e-15, -1.14095073e-15],
    [5.21849164e-15, -1.92598987e-15, 1.69405022e-15, -6.11340860e-18, 4.95368161e-16, 6.95072219e-16, 1.56088961e-14,
     -3.23455137e-15],
    [6.27820524e-15, -7.39243293e-16, -1.75734889e-15, -1.08780986e-14, 3.19246930e-15, -1.41771970e-14, 1.35153646e-15,
     -1.52447448e-14],
...
expected_y = np.fft.rfft(A * np.sin(2 * np.pi * f0 * np.arange(0, ND) * dt)) * np.sqrt(2)
phif = phitilde_vec_norm(Nf, Nt, 4.0)


def get_trasform_funct(backend):
    if backend == "numpy":
        from pywavelet.transforms.numpy.inverse.to_freq import inverse_wavelet_freq_helper_fast as inverse_wavelet_freq_helper
    elif backend == "jax":
        from pywavelet.transforms.jax.inverse.to_freq import inverse_wavelet_freq_helper
    elif backend == "cupy":
        from pywavelet.transforms.cupy.inverse.to_freq import inverse_wavelet_freq_helper
    else:
...
def test_inverse_transform(backend, plot_dir):
    ...
    assert reconstructed_signal.shape == expected_y.shape, f"Shape mismatch for backend {backend}"
    # Check if the reconstructed signal is close to the expected signal
    assert np.allclose(reconstructed_signal, expected_y, atol=1e-5), f"Inverse transform failed for backend {backend}"

    # plot the results (for visualization purposes)
    import matplotlib.pyplot as plt
    plt.figure()
    plt.plot(np.abs(expected_y)**2, label='Expected Signal', lw=2, color='black')
    plt.plot(np.abs(reconstructed_signal)**2, label='Reconstructed Signal', alpha=0.5, marker='o')
    plt.yscale('log')
