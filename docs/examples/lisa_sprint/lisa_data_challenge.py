...
def plot_mask(wnm, mask, label='mask'):
    ...
    mask_wnm.data = mask_wnm.data.at[~mask].set(0)  # set non-outliers to 0 (unmasked -- we keep)
    d = wnm.data.ravel()
    percentage = np.sum(mask) / len(d)
    print(f"Percentage of data masked: {percentage:.2%}")

    mask_wnm.plot(absolute=True, cmap='binary', show_colorbar=False, label=f"{percentage:.2%}% masked", zscale='linear')
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, f'{label}.png'))
...
def iterative_denoise(wnm: Wavelet,
                      iterations: int = 5,
